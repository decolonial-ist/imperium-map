...
# Удержания, у которых малая доля внутри контура законна: геометрия удержания -
# область-приближение, а имперской в ней была только часть. Вырез всё равно
# обрезается контуром ядра на дату, проверка «меньше 20%» здесь ловила бы не
# ошибку, а сам факт (24.09.2026, приёмка пачек 3-11 новым сборщиком).
LOW_SHARE_OK = {
    'razin_1667': ('калмыцкие кочевья - не империя до 1771 года (пачка 3, 22.09.2026): '
                   'в Астраханской области имперской была только волжская полоса с '
                   'Астраханью, её вырез и режет (15% области на срезе 1670 года)'),
}
        ...
        for n_win, w in enumerate(wins, 1):
            ...
            if clipped.is_empty:
                bad.append(f"{w['where']}: удержанная территория целиком вне "
                           ...
                           f'вычитать нечего')
                continue
            if share < 0.2 and r['name'].strip() not in LOW_SHARE_OK:
