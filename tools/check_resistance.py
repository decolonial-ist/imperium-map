...
P = {
    ...
    'Львов': (24.032, 49.842),
    'Запорожье (Александровск)': (35.139, 47.838),
    'Днепр (Екатеринослав)': (35.046, 48.465),
    'Никополь': (34.404, 47.567),
    'Пологи': (36.253, 47.478),
    'Павлоград': (35.870, 48.530),
    'Варшава': (21.012, 52.230),
    'Плоцк': (19.706, 52.547),
    'Августов': (22.979, 53.844),
    'Люблин': (22.568, 51.247),
    'Седльце': (22.290, 52.168),
    'Конин': (18.251, 52.223),
    'Радом': (21.147, 51.403),
    'Кельце': (20.629, 50.866),
    'Сачхере': (43.418, 42.345),
    'Зестафони': (43.048, 42.110),
}
...
def main():
    ...
    # окна удержания: в колонках реестра или в data/resistance/holds.csv (с
    # 19.09.2026 - когда у выступления окон несколько)
    hp = os.path.join(DATA, 'resistance', 'holds.csv')
    holds = []
    if os.path.exists(hp):
        with open(hp, encoding='utf-8', newline='') as f:
            holds = [h for h in csv.DictReader(f)
                     if h['name'].strip() and not h['name'].startswith('#')]
    ok('окна удержания: у каждого окна в holds.csv есть выступление в реестре',
       sorted({h['name'] for h in holds} - {r['name'] for r in up}), [])
    ok('окна удержания: выступление с окнами в holds.csv не держит их в реестре',
       sorted(r['name'] for r in up if r['hold_from'].strip()
              and r['name'] in {h['name'] for h in holds}), [])
    ...
    # ---- 3б. махновщина по документам (19.09.2026) --------------------------
    ok('Александровск 20.10.1919 - не красный (махновский гарнизон, ВРС)',
       red(*P['Запорожье (Александровск)'], '1919-10-20'), False)
    ok('Александровск 20.11.1919 - красный (управа отчитывается белым)',
       red(*P['Запорожье (Александровск)'], '1919-11-20'), True)
    ok('Екатеринослав 20.10.1919 - красный (махновцы вошли 07.11)',
       red(*P['Днепр (Екатеринослав)'], '1919-10-20'), True)
    ok('Екатеринослав 20.11.1919 - не красный (махновская комендатура)',
       red(*P['Днепр (Екатеринослав)'], '1919-11-20'), False)
    ok('Екатеринослав 15.12.1919 - красный (выбиты 08.12)',
       red(*P['Днепр (Екатеринослав)'], '1919-12-15'), True)
    ok('Никополь 20.12.1919 - не красный (ВРС в городе)',
       red(*P['Никополь'], '1919-12-20'), False)
    ok('Пологи 10.11.1919 - не красные (штаб второй группы)',
       red(*P['Пологи'], '1919-11-10'), False)
    ok('Павлоград 01.11.1919 - красный: удержания нет, был только удар по '
       'станции Синельниково',
       red(*P['Павлоград'], '1919-11-01'), True)

    # ---- 3в. Ноябрьское восстание по воеводствам (19.09.2026) ----------------
    # даты - старый стиль, как у всей карты до 1918 года
    ok('Варшава 01.11.1830 - красная (до восстания)',
       red(*P['Варшава'], '1830-11-01'), True)
    ok('Варшава 01.05.1831 - не красная (Национальное правительство)',
       red(*P['Варшава'], '1831-05-01'), False)
    ok('Варшава 01.09.1831 - красная (войска вошли 27.08)',
       red(*P['Варшава'], '1831-09-01'), True)
    ok('Плоцк 01.09.1831 - не красный (последняя резиденция сейма)',
       red(*P['Плоцк'], '1831-09-01'), False)
    ok('Плоцк 20.09.1831 - красный (армия и правительство ушли 15.09)',
       red(*P['Плоцк'], '1831-09-20'), True)
    ok('Августов 01.02.1831 - не красный',
       red(*P['Августов'], '1831-02-01'), False)
    ok('Августов 10.03.1831 - красный (Гаврыхи 23.02)',
       red(*P['Августов'], '1831-03-10'), True)
    ok('Люблин 01.03.1831 - не красный',
       red(*P['Люблин'], '1831-03-01'), False)
    ok('Люблин 01.06.1831 - красный (Ридигер держит Люблин к 16.05)',
       red(*P['Люблин'], '1831-06-01'), True)
    ok('Седльце 10.01.1831 - не красное',
       red(*P['Седльце'], '1831-01-10'), False)
    ok('Седльце 10.02.1831 - красное (операционная база с 24.01)',
       red(*P['Седльце'], '1831-02-10'), True)
    # Конин, а не Калиш: сам Калиш лежит за краем контура ядра у прусской
    # границы и не красный ни до восстания, ни после (дыра канона, Б6)
    ok('Конин 01.08.1831 - не красный (Калишское воеводство)',
       red(*P['Конин'], '1831-08-01'), False)
    ok('Конин 15.08.1831 - красный (Калиш занят Кноррингом 09.08)',
       red(*P['Конин'], '1831-08-15'), True)
    ok('Радом 20.07.1831 - не красный',
       red(*P['Радом'], '1831-07-20'), False)
    ok('Радом 05.08.1831 - красный (занят 01.08)',
       red(*P['Радом'], '1831-08-05'), True)
    ok('Кельце 10.08.1831 - не красное (отряд Ружицкого)',
       red(*P['Кельце'], '1831-08-10'), False)
    ok('Кельце 25.08.1831 - красное (после 18.08)',
       red(*P['Кельце'], '1831-08-25'), True)

    # ---- 3г. Имеретия 1810 (19.09.2026, АКАК т. IV) --------------------------
    ok('Сачхере 01.06.1810 - не красное (царь Соломон II)',
       red(*P['Сачхере'], '1810-06-01'), False)
    ok('Зестафони 01.09.1810 - не красное',
       red(*P['Зестафони'], '1810-09-01'), False)
    ok('Сачхере 15.10.1810 - красное (Ханское ущелье 25.09)',
       red(*P['Сачхере'], '1810-10-15'), True)
    ...
    ok('Оренбург 01.06.1774 - красный: осаду город выдержал',
       ...
       'город не сдался; окна Пугачёва вокруг Оренбурга - розыск 19.09.2026, '
       'MAP-MATERIALS/peresmotr_resheniy_2026-09-18/rozysk/b_pugachev_orenburg.md')
    ok('Казань 01.08.1774 - красная: захвачена на сутки',
       red(*P['Казань'], '1774-08-01'), True)
    ok('Уфа 01.06.1774 - красная: город осаду выдержал',
    ...
    ok('Тобольск 01.03.1921 - вырезов пока нет: окно 21.02-08.04.1921 '
       'ещё не заведено (розыск 19.09.2026)',
       cut_names(*P['Тобольск'], '1921-03-01'), [])
    ok('Астрахань 01.01.1706 - вырезов нет: армейский мятеж гарнизона',
       cut_names(*P['Астрахань'], '1706-01-01'), [],
       'город держал гарнизон самой империи от имени царя - решение куратора '
       '19.09.2026')
